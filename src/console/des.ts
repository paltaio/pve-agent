/**
 * DES for the RFB security type 2 challenge.
 *
 * The server sends a 16-byte challenge and the client answers with it
 * encrypted under DES-ECB, keyed by the password cut or zero-padded to 8 bytes.
 * VNC keys DES with the bits of each key byte reversed, so the seven ASCII
 * bits of each character land in the bits DES reads.
 */
...
/** Selects bits by 1-based position. */
function permute(bits: Bits, table: readonly number[]): Bits {
	return table.map((position) => bits[position - 1] ?? 0)
}
...
function fromBits(bits: Bits): Buffer {
	const bytes: number[] = []
...
function rotateLeft(half: Bits, count: number): Bits {
	return [...half.slice(count), ...half.slice(0, count)]
}
...
function feistel(right: Bits, subkey: Bits): Bits {
	const expanded = xor(permute(right, E), subkey)
	const out: Bits = []
	for (let box = 0; box < 8; box++) {
		const six = expanded.slice(box * 6, box * 6 + 6)
		const row = ((six[0] ?? 0) << 1) | (six[5] ?? 0)
		const column =
			((six[1] ?? 0) << 3) | ((six[2] ?? 0) << 2) | ((six[3] ?? 0) << 1) | (six[4] ?? 0)
		const value = SBOX[box]?.[row * 16 + column] ?? 0
		for (let i = 3; i >= 0; i--) out.push((value >> i) & 1)
	}
	return permute(out, P)
}

/** Encrypts one 8-byte block with DES-ECB under an 8-byte key. */
export function desEncryptBlock(key: Uint8Array, block: Uint8Array): Buffer {
	...
	return fromBits(permute([...right, ...left], FP))
}
...
/** Encrypts a 16-byte VNC challenge as two independent DES-ECB blocks. */
export function vncDesEncrypt(password: string, challenge: Uint8Array): Buffer {
	const key = Buffer.alloc(8)
	const ascii = Buffer.from(password, 'ascii')
	for (let i = 0; i < 8; i++) key[i] = reverseBits(ascii[i] ?? 0)
